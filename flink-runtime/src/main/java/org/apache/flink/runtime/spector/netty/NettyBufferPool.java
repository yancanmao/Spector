...
/*
	private final ConcurrentLinkedQueue<ByteBuf> allocatedBuffers = Queues.newConcurrentLinkedQueue();

	public void checkReleased() {
		for (ByteBuf buf : allocatedBuffers) {
			String str = buf.toString();
			if (!str.contains("freed")) {
				LOG.error("Buffer {} of class {} not released!", str, buf.getClass().getSimpleName());
				try {
					Field leakField = buf.getClass().getDeclaredField("leak");
					leakField.setAccessible(true);
					Field modifiersField = Field.class.getDeclaredField("modifiers");
					modifiersField.setAccessible(true);
					modifiersField.setInt(leakField, leakField.getModifiers() & ~Modifier.FINAL);
					LOG.error("{}", leakField.get(buf));
	...
	public ByteBuf directBuffer() {
		ByteBuf buff = super.directBuffer();
		allocatedBuffers.add(buff);
		return buff;
	}

	@Override
	public ByteBuf directBuffer(int initialCapacity) {
		ByteBuf buff = super.directBuffer(initialCapacity);
		allocatedBuffers.add(buff);
		return buff;
	}
	...
	public ByteBuf directBuffer(int initialCapacity, int maxCapacity) {
		ByteBuf buff = super.directBuffer(initialCapacity, maxCapacity);
		allocatedBuffers.add(buff);
		return buff;
	}
*/
