...
public abstract class StateBackendTestBase<B extends AbstractStateBackend> extends TestLogger {
	...
	public void testBackendUsesRegisteredKryoDefaultSerializer() throws Exception {
		...
		try {
			...
			runSnapshot(
				backend.snapshot(682375462378L, 2, streamFactory, CheckpointOptions.forCheckpointWithDefaultLocation(), true, new HashSet<>()),
				sharedStateRegistry);
		} catch (ExpectedKryoTestException e) {
			numExceptions++;
		} catch (Exception e) {
		...
		try {
			...
			runSnapshot(
				backend.snapshot(
					...
					2,
					streamFactory,
					CheckpointOptions.forCheckpointWithDefaultLocation(), true, new HashSet<>()), sharedStateRegistry);
	...
	public void testValueState() throws Exception {
		...
		KeyedStateHandle snapshot2 = runSnapshot(
			backend.snapshot(682375462379L, 4, streamFactory, CheckpointOptions.forCheckpointWithDefaultLocation(), true, new HashSet<>()),
			sharedStateRegistry);
	...
	public void testSnapshotNonAccessedState() throws Exception {
		...
		try {
			...
			KeyedStateHandle snapshot = runSnapshot(
				backend.snapshot(1L, 2L, streamFactory, CheckpointOptions.forCheckpointWithDefaultLocation(), true, new HashSet<>()),
				sharedStateRegistry);
			backend.dispose();
			...
			snapshot = runSnapshot(
				backend.snapshot(2L, 3L, streamFactory, CheckpointOptions.forCheckpointWithDefaultLocation(), true, new HashSet<>()),
				sharedStateRegistry);
			backend.dispose();
	...
	public void testKeyGroupSnapshotRestore() throws Exception {
		...
		KeyedStateHandle snapshot = runSnapshot(
			backend.snapshot(0, 0, streamFactory, CheckpointOptions.forCheckpointWithDefaultLocation(), true, new HashSet<>()),
			sharedStateRegistry);
	...
	public void testEmptyStateCheckpointing() {
		...
		try {
			...
			KeyedStateHandle snapshot =
				runSnapshot(
					backend.snapshot(682375462379L, 1, streamFactory, CheckpointOptions.forCheckpointWithDefaultLocation(), true, new HashSet<>()),
					sharedStateRegistry);
			assertNull(snapshot);
			backend.dispose();
	...
	public void testParallelAsyncSnapshots() throws Exception {
		...
		try {
			...
			RunnableFuture<SnapshotResult<KeyedStateHandle>> snapshot1 =
				backend.snapshot(0L, 0L, streamFactory, CheckpointOptions.forCheckpointWithDefaultLocation(), true, new HashSet<>());
			...
			RunnableFuture<SnapshotResult<KeyedStateHandle>> snapshot2 =
				backend.snapshot(1L, 1L, streamFactory, CheckpointOptions.forCheckpointWithDefaultLocation(), true, new HashSet<>());
		...
		try {
			...
			for (int i = 0; i < 10; ++i) {
				...
				futureList.add(runSnapshotAsync(executorService,
					backend.snapshot(checkpointID++, System.currentTimeMillis(), streamFactory, CheckpointOptions.forCheckpointWithDefaultLocation(), true, new HashSet<>())));
			}
