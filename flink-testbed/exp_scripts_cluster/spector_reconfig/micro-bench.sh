...
function analyze() {
    ...
    for host in $hosts
    do
        scp $host:${FLINK_DIR}/log/* ${EXP_DIR}/spector/log
        scp $host:${FLINK_DIR}/spector/* ${EXP_DIR}/spector/$host
    done 
    ...
    for host in $hosts
    do
        ssh $host "mkdir ${EXP_DIR}/spector/"
    done 
}
...
#  for sync_keys in 1 2 4 8 16 32 64 128 256; do
  for sync_keys in 1 2 4 8 16 32 64 128; do # 1 2 4 8 16 32 64 128
    run_one_exp
  done
}
