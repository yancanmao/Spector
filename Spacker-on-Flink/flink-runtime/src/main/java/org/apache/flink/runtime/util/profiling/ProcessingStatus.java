...
public class ProcessingStatus implements Serializable {
	...
	void incRecordsOutChannel(int targetKeyGroup) {
		if (!outputKeyGroup.containsKey(targetKeyGroup)) {
			outputKeyGroup.put(targetKeyGroup, outputKeyGroupState.getOrDefault(targetKeyGroup, 0l));
//			outputKeyGroup.put(targetKeyGroup, outputKeyGroupState.get(targetKeyGroup));
		}
		outputKeyGroup.put(targetKeyGroup, outputKeyGroup.get(targetKeyGroup)+1);
	}

	void incRecordsIn(int keyGroup) {
		if (!inputKeyGroup.containsKey(keyGroup)) {

			inputKeyGroup.put(keyGroup, inputKeyGroupState.getOrDefault(keyGroup, 0l));
//			inputKeyGroup.put(keyGroup, inputKeyGroupState.get(keyGroup));
		}
		inputKeyGroup.put(keyGroup, inputKeyGroup.get(keyGroup)+1);
	}
}
